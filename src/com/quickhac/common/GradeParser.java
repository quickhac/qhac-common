...
public class GradeParser {
	...
	public Course[] parseAverages(final String html) {
		...
		final Elements $headerCells = $gradeTable.select("tr.TableHeader")
				.first().getElementsByTag("th");
		...
		String text;
		semParams.hasExams = false;
		semParams.hasSemesterAverages = false;
		for (Element $cell : $headerCells) {
			...
			if (cycleMatcher.matches()) {
				semParams.cyclesPerSemester = Integer.valueOf(cycleMatcher
						.group(1));
				continue;
			}
		...
		if (semParams.semesters == 0)
			semParams.semesters = 1;
		...
		boolean cumulative = $rows.get(0).getElementsByClass("TeacherNameCell").first().text().equals("Cumulative GPA");
		courses = cumulative ? new Course[$rows.size() - 1] : new Course[$rows.size()];
 
		// parse each course
		for (int i = 0; i < courses.length; i++) {
			if(cumulative) {
				courses[i] = parseCourse($rows.get(i + 1), semParams);
			} else {
	...
	Course parseCourse(final Element $row, final SemesterParams semParams) {
		...
		final Element $teacherCell = $row.getElementsByClass("EmailLink")
				.first();
		...
		for (int i = 0; i < semParams.semesters; i++) {
			...
			final Element $examCell = semParams.hasExams ? $cells
					.get(cellOffset + semParams.cyclesPerSemester) : null;

			// and semester cell is after that
			final Element $semAvgCell = semParams.hasSemesterAverages ? $cells
	...
	Semester parseSemester(Element[] $cycles, Element $exam, Element $semAvg,
		...
		if (semParams.hasSemesterAverages) {
			...
			if (parsedSemAvg.type == GradeValue.TYPE_LETTER)
				...
				semester.average = GradeCalc.semesterAverage(semester,
						district.examWeight());
		} else {
	...
	Cycle parseCycle(Element $cell, int index) {
		...
		final Matcher urlHashMatcher = GRADE_CELL_URL_REGEX.matcher($link
				.first().attr("href"));
